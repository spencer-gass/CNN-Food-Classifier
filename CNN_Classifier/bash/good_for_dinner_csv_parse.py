...
for img_biz_pair in image_to_biz_csv:
    ...
    if label_of_interest in biz_to_label[biz_id]:
        i = int((len(biz_to_label[biz_id])+1)/2)
        competing_label_cnt[i] += 1
...
print(competing_label_cnt)

with open('good_for_dinner.txt','w') as gfile, open('bad_for_dinner.txt','w') as bfile:
